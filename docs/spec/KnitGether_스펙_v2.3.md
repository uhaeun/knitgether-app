...
- **CR-3 (Seed 데이터)**: 신규 사용자/환경이 기획에 명시된 기본 스킬·사전 데이터를 갖춰야 한다. → **[Issue #11](../../issues/11) (Resolved, verify 대기)**
  - **스킬**: §22의 15종은 **필수**이되 **시스템 확장 허용**으로 현행화. 서버는 시스템 스킬 41종(§22 15종 전부 포함 + 블로그 확장)을 `isSystem=true`로 보유하고 모든 사용자에게 병합 노출. 누락됐던 표기성 4종(RS/WS/Rep/St)은 seed로 보강.
  - **사전**: §23의 18종을 `isSystem=true` 시스템 용어로 seed. `DictionaryTerm.isSystem` 컬럼 도입 + 조회 쿼리 병합(스킬과 대칭)으로 모든 사용자에게 노출. 시스템 용어는 사용자 수정/삭제 불가(403).
  - **seed 스크립트**: `server/scripts/seed-system-content.ts`(`npm run seed`), 자연키 기반 멱등(§32 #16 준수 — 재실행 시 중복 생성 없음).
