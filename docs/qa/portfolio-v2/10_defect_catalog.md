...
> 2026-09-18 재작성. 결함 ID, 제목, 결함 클래스, 심각도 판정, 발견 경위는 원본(`docs/qa/portfolio/10`, 2026-09-06 갱신분까지)에서 옮겼다. 원본 대장의 상태 열은 구글시트 결함 대장 8/24 기준이라 이 판에서는 상태를 2026-09-18 GitHub Issue 기준으로 다시 적었다. 2026-09-19에 원본 9/15 갱신분(#14 잔존 2건, DEF-23 재현 불가 확정, 남은 판단 7번과 8번)을 반영했다. 근거는 `docs/qa/fix_cycle_worklog_2026-09-03.md`와 각 이슈의 수정 확인 코멘트다. 8/24 시점의 상태는 6절 이력에 남겼다. 재현 절차 전문, psql 원문, 발견 시각은 06과 그 부록이 정본이다.
...
DEF-01은 인수인계 문서 known issue #16의 동작을 실측으로 확정한 것이다. 문서에 적혀 있던 것과 실제로 데이터가 사라지는 것을 확인한 것은 구분하고, 이 대장은 후자를 QA 신규 발견으로 센다. DEF-23은 반대 방향의 정정이다. 9/1에 QA 신규 발견으로 적었다가 핸드오프 이슈 #9가 같은 증상임을 확인하고 핸드오프 known issue로 옮겼다(4절 DEF-23, 6절).
...
| DEF-23 | 앱 백그라운드 진입 시 작업 세션이 종료되지 않음 | RC-04 카운트 부정확 | Critical (9/1 확정) | 7/29 핸드오프 known issue (#9). 9/1 코드 대조로 미수정 확인, TIME-02로 스펙화 | f93aacc 대조 중 scenePhase 부재 재확인 |
...
| DEF-15 | `34f3c00` 겹침 제거 (8/6) | UI-39, UI-54, CI 게이트 `testDEF15` | #32 | 8/30 | 있음 |
...
| DEF-20 | `1637acc` 행 지시가 있을 때만 400 (9/5 보고) | `test_05_transaction_atomicity.py::test_unknown_row_counter_id_is_rejected_without_mutation`. 400, 에러 코드, 부모 프로젝트 미변경(psql) 확인 | #38 | 9/6 | 있음 |
| DEF-21 | `e1b25ad` (8/10) | UI-46, TC-CUJ3-01 | #39 | 8/30 | 있음 |
| DEF-22 | `1637acc` `@IsDefined()` 추가 | `test_16_coverage_promotions.py::test_api_07_missing_row_counter_is_rejected_without_insert`. 거부의 원자성을 psql로 확인 | 미등록 | 해당 없음 | 해당 없음 |
| DEF-23 | `4dea734` scenePhase 연결 (9/5 보고) | ProjectWorkspaceViewModelTests 80 passed, 그중 `backgroundEndsSessionAndActivationStartsANewOne`. Appium 재현 케이스 두 빌드 대조 (4절). 9/15 짧은 백그라운드 케이스 신설, 원래 증상은 재현 불가 확정 `1362481` | #9 (#40은 중복) | 9/6 | 없음 |
| DEF-24 | `0335515` 입력 보존과 문구 | ProjectWorkspaceViewModelTests | #41 | 9/6 | 없음 |
| DEF-25 | `f685bc1` 연결 기준 통일 | library.e2e, LibraryItemViewModelTests | #42 | 9/6 | 없음 |
| DEF-26 | `52dee72` 단위 선택 제거 | UI-01, OnboardingViewModelTests | #43 | 9/6 | 없음 |
| DEF-27 | `982d858` 80자 상한 | 앱 단위 335 passed | #44 | 9/6 | 없음 |
| DEF-28 | `982d858` 문구 교체 | 앱 단위 335 passed | #45 | 9/6 | 없음 |
| DEF-29 | `52dee72` 단위 선택 제거, `c40836d` 메뉴 부제 | UI-01, 시뮬레이터 확인 | #46 | 9/6 | 없음 |
...
DEF-23과 DEF-24~29도 verified 라벨이 없다. 닫는 코멘트에 수정 커밋과 검증 결과(빌드 통과, 단위 테스트 통과 건수, UI-01)는 있으나 라벨은 붙지 않았다. DEF-22는 이슈로 등록되지 않은 채 수정됐다.
...
![DEF-08 재현: 500 vs 200](../portfolio/evidence/0830_DEF-08_repro_500_vs_200.png)

![DEF-08 앱 화면 전후](../portfolio/evidence/0830_DEF-08_app_before_after.png)
...
![DEF-10 재현 전후](../portfolio/evidence/0830_DEF-10_drawing_loss_before_after.png)
...
![DEF-16 재현: login 200 반복, 앱은 미로그인](../portfolio/evidence/0830_DEF-16_login_race_old_build.png)
...
<!--
원본과 최신 기록의 차이 (2026-09-18)
1. 원본 2절 상태 열은 구글시트 8/24 기준(DEF-17~19 미수정, SPEC-PROJ-01 미구현, DEF-20~29 시트 미등록). 이 판은 2026-09-18 GitHub Issue 상태로 적었다.
2. DEF-20: 원본 대장 "미수정", 회귀 감시 `test_unknown_row_counter_id_leaks_500`, 5절 4번도 미수정 전제. #38 코멘트(9/5 수정 보고)와 fix_cycle 워크로그 2절 기준 `1637acc`로 수정, #38 9/6 닫힘 verified. 회귀 케이스는 `test_unknown_row_counter_id_is_rejected_without_mutation`.
3. DEF-22: 원본 대장 "미수정", 이슈 "미등록". fix_cycle 워크로그 2절과 `1637acc` 커밋 제목 기준 수정. 회귀 케이스 이름 `test_api_07_missing_row_counter_is_rejected_without_insert`는 2026-09-18 qa/api-tests 파일에서 확인했다. 이슈는 여전히 없다.
4. DEF-23: 원본 대장 "미수정", 회귀 "미정 (TIME-02 TC 신설 필요)". `4dea734`, #9 9/6 닫힘. 워크로그 3절 기준.
5. #40: 원본 대장은 "#40은 9/1 중복 등록, 닫음". 이슈 기록상 #40의 닫힌 시각과 중복 코멘트는 2026-09-06이다.
6. DEF-05~07: 원본 대장 행은 "수정 완료 (9/6)"로 갱신됐으나 4-2절(open 4)과 5절 2번(RC-02 잔존 3건 릴리즈 판정)은 갱신 전 상태. 9ba16c9와 #22~#24 코멘트 기준으로 닫힘.
7. 원본 대장의 DEF-24~29 행은 칸이 하나 밀려 있다(상태 칸에 "시트 미등록"과 "수정 완료 (9/6)"가 연속). 이 판에서는 수정 커밋과 회귀 칸을 맞춰 옮겼다.
8. 원본 2-1절 8/29 판정표의 CUJ-4는 9/1 재분류 전 번호다. 지금의 CUJ-3(모르는 기술 만남)이다.
9. DEF-23의 결함 클래스 "RC-04 카운트 부정확"은 원본 표기다. 03의 RC-04는 데이터 계약 불일치라 명칭이 어긋난다. 판정을 바꾸지 않고 원본 표기를 유지했다.
10. verified 라벨: #9, #22~#24, #41~#46에 없다. 원본 10에는 라벨에 대한 서술이 없다. #22~#24는 코멘트에 단위 테스트 판정 사유가 있고, #9와 #41~#46은 라벨이 없는 사유가 기록에 없다.
11. 원본 1절 표는 "QA 사이클 중 발생한 회귀", "명세 공백"을 세 범주와 별도 행으로 뒀다. 이 판은 DEF-15를 QA 신규 발견 아래 세부 경위로 두고, SPEC-PROJ-01은 결함이 아니므로 명세 공백 행으로 따로 뒀다.
12. 원본 상단의 구글시트 URL은 옮기지 않았다.
13. DEF-08 수정 커밋: 원본 10, 06, 09는 `06c7a10`으로 적었으나 리포지토리에 없는 해시다(2026-09-19 `git cat-file` 확인). 2026-08-06 커밋 `7defe1c` "fix(projects): 목록 조회에서 자식 레코드 결손 시 해당 프로젝트만 제외"가 본문에 "TC-CNT03-01 FAIL 수정"을 적고 있어 이 해시로 고쳤다. 원본의 "8/27 전건 실재 확인"은 이 해시에 대해서는 성립하지 않았다.
14. 원본 10의 5절(남은 판단) 7번(잔존 결함 전건 처리, 9/15 해소)과 8번(DEF-23 짧은 백그라운드 케이스의 검출력)은 이 판에 남은 판단 절이 없어 6절 이력과 7절 간격 표로 옮겼다. 원본 5절 2번(RC-02 잔존 3건의 릴리즈 판정)은 원본에서도 9/6 해소로 정정됐다.
-->
