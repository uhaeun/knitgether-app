...
| API-02 | 비밀번호 길이 규칙의 서버 거부 | 없음 | 미커버 |
| API-03 | 중복 이메일 가입 거부 | 없음 | 미커버 |
| API-04 | 로그인 성공과 실패 자격 거부 | 없음 | 미커버 |
...
| API-08 | 허용되지 않은 필드 거부 | 없음 | 미커버 |
| API-09 | 타입 오류 거부 | `test_06` 음수 currentRow (타입 아닌 값 범위) | 부분 |
...
| API-18 | stale PATCH가 기존 세션을 삭제하지 않는지 | 없음 | 미커버 |
| API-19 | 서버 거부 시 localOnly 보존 | 없음 | 미커버 |
| API-20 | 수정본 거부 시 무통보 폐기 금지 | 없음 | 미커버 |
...
집계는 덮임 8건, 부분 5건, 미커버 12건이다.

미커버 12건은 두 갈래로 갈린다. 인증 계열 4건(API-02, 03, 04, 08)은 계약 검증이라 짧게 채울 수 있다. 결함 회귀 계열 7건(API-18~24)은 각각 재현 조건을 만들어야 해서 비용이 크다. API-17 창고 CRUD는 그 중간이다.
