...
이 사이클에서 신설한 Appium 케이스 중 셋이 UI-NN 번호를 받지 못했다(2026-09-19 표 정정). 번호 배정은 카탈로그의
...
신설 일자는 커밋일이다. 8/29 실행 80건(함수 78개)에서 9/6 실행 82건으로 는 2건이 위 표의 65초 케이스와 `test_skill_cache_pruning`이고, 9/15의 83건은 8초 케이스가 더해진 것이다.
...
| API-21 | 결함 회귀 | TC4 | 프로젝트 | RowCounter 없는 프로젝트의 목록 격리 (DEF-08) | 에러 기대 | 200 + 해당 프로젝트만 제외 + 로그 기록 | 7defe1c 수정 | 수동 | psql로 RowCounter 없는 행 삽입 뒤 목록 200, 해당 건 제외, 로그 확인 [?] |
...
### API 케이스와 실행 코드 대조 (2026-08-29, 2026-09-19 행 갱신)

`qa/api-tests` 9개 파일이 25건 중 어디를 덮는지 대조했다. 덮임 판정은 그 케이스의 검증 내용을 실제로 단언하는 테스트가 있는지 기준이다. 부수적으로 지나가기만 하는 것은 부분으로 본다.

| API | 검증 내용 | 담당 테스트 | 상태 |
| --- | --- | --- | --- |
| API-01 | 회원가입 성공 시 계정 생성과 토큰 발급 | `conftest._register` (픽스처) + `test_16` UserAccount 행 대조 (9/1 보강) | 덮임 |
...
**2026-09-15 기준 집계는 덮임 27건, 부분 2건, 미커버 0건이다(2026-09-19 정정, 테스트 소스 행 대조).** 미커버 5건은 9/1 커버리지
보강에서 전건 채웠고(API-17은 `test_12`, 21~24는 `test_13`~`15`), 이후 신설 4건(API-26~29)이
더해졌다. 부분 5건 중 API-01, 07, 15는 같은 보강의 `test_16`이 검증 내용을 직접 단언해 덮임이 됐다.
부분에 남은 것은 API-12와 API-19이며 사유는 아래와 같다.
...
API-12는 판정 기준이 규칙마다 400인데, 9/1에 생긴 세션 길이 24시간 상한과 메모 500자 상한을 단언하는 API 테스트가 없다.
...
8/29 시점의 미커버 5건은 우선순위가 낮아 의도적으로 뺀 것들이었다. API-21(DEF-08, RC-04)은 P2, API-22(DEF-09, RC-07)는 P3, API-23과 24(DEF-13, 14)는 03에서 치명도 축 밖 운영 리스크로 분류된 파일 정합성 계열이다. API-17 창고 CRUD가 나머지 한 건이다. 다섯 건 모두 9/1에 채웠다.

P1만 골라 채운 것은 04의 우선순위를 따른 것이다. 자르는 근거가 우선순위에 있다는 점이 남기지 않은 것보다 중요하다.

8/29 시점의 부분 5건 중 셋은 9/1 `test_16` 보강으로 덮임이 됐다(2026-09-19 테스트 소스 대조). 부수적으로 지나가는 것을 커버로 셀지가 판단이고, 이 문서가 다른 문서에 인용될 때 숫자가 달라지는 지점이다.
