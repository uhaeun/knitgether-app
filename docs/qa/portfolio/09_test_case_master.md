...
집계는 2026-08-29 기준 덮임 13건, 부분 4건, 미커버 8건이다.

초기 대조에서는 덮임 8건, 부분 5건, 미커버 12건이었다. 비용이 낮은 인증 계약 5건(API-02, 03, 04, 08, 09)을 `test_10_auth_contract.py` 로 채워 덮임이 13건이 됐다.

남은 미커버 8건 중 7건이 결함 회귀 계열(API-18~24)이다. 각각 재현 조건을 만들어야 해서 비용이 크고, 07을 마무리하기 전에 어디까지 갈지 정해야 하는 지점이다. API-17 창고 CRUD가 나머지 한 건이다.
