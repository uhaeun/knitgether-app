...
| 항목 | 값 |
|---|---|
| 앱 코드 기준 | SMK-004/005/006/008/009/010, 프로젝트 수정/삭제, 서버 OFF cache 없음 UI 자동화 포함 |
| 브랜치 | `main` |
...
| iOS UI | PASS | UI test 12개, launch matrix 포함 15 passed runs |
...
| 앱 사용 가능성 | 제한적 PASS. 회원가입/스킬 테스트/프로젝트 생성/수정/삭제/재실행 유지/작업공간 단수/작업시간/게이지 기본 흐름, 오프라인 cache/pending 복구/cache 없음 안내, 계정 cache 분리는 자동화로 통과 |
| 릴리즈 판정 | 아직 No-Go. RowInstruction 저장/삭제 수동 QA가 남음 |

## 다음 실행 우선순위

| 순서 | 대상 | 이유 |
|---|---|---|
| 1 | RowInstruction 저장/삭제 | child pending retry와 삭제 재시도 결함 이력 확인 필요 |
| 2 | 기존 계정 로그인 재진입 | 재방문 사용자의 진입 흐름 확인 필요 |
| 3 | 로그아웃 후 프로필 오류 미노출 | 최근 수동 사용 중 관찰된 사용자 혼란 포인트 |
