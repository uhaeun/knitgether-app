...
| 항목 | 값 |
|---|---|
| 앱 코드 기준 | SMK-004 프로젝트 재실행 유지 UI 자동화 포함 |
| 브랜치 | `main` |
...
| iOS UI | PASS | UI test 5개, launch matrix 포함 8 passed runs |
...
| 모든 P0가 실행됐는가 | NO |
| 앱 사용 가능성 | 제한적 PASS. 회원가입/스킬 테스트/프로젝트 생성/프로젝트 재실행 유지/게이지 기본 흐름은 자동화로 통과 |
| 릴리즈 판정 | 아직 No-Go. 작업공간, 오프라인 retry, 계정 전환 수동 QA가 남음 |

## 다음 실행 우선순위

| 순서 | 대상 | 이유 |
|---|---|---|
| 1 | SMK-005/006 작업공간 단수/작업시간 | 사용자가 매일 쓰는 핵심 기록 |
| 2 | SMK-008/009 오프라인 cache/pending retry | 이 앱의 offline-first 핵심 리스크 |
| 3 | SMK-010 계정 전환 | cache scope와 개인정보 리스크 |
