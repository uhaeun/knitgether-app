...
| 항목 | 값 |
|---|---|
| 앱 코드 기준 | SMK-004/005/006/008/009 UI 자동화 포함 |
| 브랜치 | `main` |
...
| iOS UI | PASS | UI test 9개, launch matrix 포함 12 passed runs |
...
| 앱 사용 가능성 | 제한적 PASS. 회원가입/스킬 테스트/프로젝트 생성/프로젝트 재실행 유지/작업공간 단수/작업시간/게이지 기본 흐름, 오프라인 cache/pending 복구는 자동화로 통과 |
| 릴리즈 판정 | 아직 No-Go. 계정 전환, 프로젝트 수정/삭제, cache 없음 오프라인 UX 수동 QA가 남음 |

## 다음 실행 우선순위

| 순서 | 대상 | 이유 |
|---|---|---|
| 1 | SMK-010 계정 전환 | cache scope와 개인정보 리스크 |
| 2 | 프로젝트 수정/삭제 | 데이터 변경과 삭제는 결함 영향이 큼 |
| 3 | cache 없음 오프라인 UX | 서버 OFF 최초 진입 안내 문구와 복구 경로 확인 필요 |
