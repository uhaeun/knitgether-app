...
| DEF-24 | 저장 충돌 안내 문구가 동작과 모순 ("다시 시도" 요구, 수정본은 폐기) | RC-03 표시 정합성 | Minor (9/1 확정) | 9/1 (정적 점검) | f93aacc 낙관적 잠금 대조 | 시트 미등록 | 수정 완료 (9/6) | `0335515` 입력 보존 + 문구 | ProjectWorkspaceViewModelTests | #41 |
| DEF-25 | 창고 삭제 고지가 사용 기록 기준, 바늘과 도구는 고지 없음 | RC-03 표시 정합성 | Minor (9/1 확정) | 9/1 (정적 점검) | 기능정의서 정의 없음 14번 승인 중 코드 대조 | 시트 미등록 | 수정 완료 (9/6) | `f685bc1` 연결 기준 통일 | library.e2e + LibraryItemViewModelTests | #42 |
| DEF-26 | 단위 옵션 값 체계가 화면별로 다름 (온보딩 US, 그 외 Imperial) | RC-04 데이터 계약 | Minor (9/1 확정, UI-03 기준) | 9/1 (정적 점검) | 기능정의서 도구, 설정, 홈 작성 중 대조 | 시트 미등록 | 수정 완료 (9/6) | `52dee72` 단위 선택 제거 | UI-01 + OnboardingViewModelTests | #43 |
| DEF-27 | 프로필 편집의 표시 이름에 80자 클라 상한 없음 (회원가입엔 있음) | RC-04 데이터 계약 | Minor (9/1 확정) | 9/1 (정적 점검) | AppInputLimit 적용처 전수 확인 | 시트 미등록 | 수정 완료 (9/6) | `982d858` 80자 상한 | 앱 단위 | #44 |
| DEF-28 | 스킬 테스트 저장 실패 문구가 개발자용 (Xcode 콘솔, 서버 Terminal) | RC-03 표시 정합성 | Minor (9/1 확정) | 9/1 (정적 점검) | 기능정의서 도구 작성 중 대조 | 시트 미등록 | 수정 완료 (9/6) | `982d858` 문구 교체 | 앱 단위 | #45 |
| DEF-29 | 설정 단위 설명문이 미적용 사실과 어긋남 ("게이지와 길이 입력에서 사용") | RC-03 표시 정합성 | Minor (9/1 확정, UI-03 기준) | 9/1 (정적 점검) | UI-03 신설과 함께 확정 | 시트 미등록 | 수정 완료 (9/6) | `52dee72` 단위 선택 제거 + `c40836d` 메뉴 부제 | UI-01 + 시뮬레이터 확인 | #46 |
