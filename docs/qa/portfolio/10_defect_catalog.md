...
| ID | 제목 | 분류 | 심각도 | 발견 시점 | 발견 경위 | 상태 (시트 8/24) | 수정 커밋 (repo 실측 8/27) | 회귀 담당 | 이슈 |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
...
| DEF-05 | 계정 전환 시 타 소유자 데이터 노출 | RC-02 계정 격리 | Critical | 8월 초 (06 수동) | TC-AUTH04-01 실행 | **부분 수정 (잔존)** | 캐시 프루닝 계열 | UI-77 | #22 |
| DEF-06 | 로그아웃 시 익명 영역으로 캐시 유입 | RC-02 계정 격리 | Critical | 8월 초 (탐색 관찰) | DEF-05 조사 중 캐시 폴더 실측 | **부분 수정 (잔존)** | `4d76203` AUTH-06 게이트 (8/11) | UI-77 | #23 |
| DEF-07 | 타 소유자 프로젝트 화면 표시 (업로드는 격리됨) | RC-02 계정 격리 | Critical | 8월 초 (06 수동) | TC-AUTH05-01 실행 | **부분 수정 (잔존)** | DEF-05와 동일 계열 | UI-77 | #24 |
| DEF-08 | RowCounter 없는 프로젝트가 목록 전체 500 | RC-04 데이터 계약 | **Critical** (8/29 판정) | 8월 초 (06 수동) | TC-CNT03-01 실행 | 수정 완료 (회귀 확인) | `06c7a10` 목록 격리 (8/6) | API-21 | #25 |
...
| DEF-15 | 카운터 0에서 감소가 증가로 오동작 | 회귀 (UX 개편 유발) | **Major** (8/29 판정) | 8월 초 (06 실행 중) | TC-CNT01-01 실행 | 수정 완료 (자동화) | `34f3c00` 겹침 제거 (8/6) | UI-39, UI-54 + CI 게이트 | #32 |
...
| DEF-17 | 작업 세션 저장 시 lastWorkedAt 미갱신 | RC-03 표시 정합성 + 데이터 계약 | Major | 8/23 (케이스 설계) | UI-33 설계 중 정렬 관찰 → 코드 대조 → `projects.json` 실증 | 미수정 | **`543ae7f` (8/26, 시트 미반영)** | UI-33, 35, 38, API-25 | #34 |
| DEF-18 | 보기 모드에서 드로잉 미표시 | RC-03 표시 정합성 | Major | 8/23 (케이스 설계) | UI-48 실행 중 관찰 | 미수정 | **`d1c6010` (8/26, 시트 미반영)** | UI-49 | #35 |
| DEF-19 | 탭 복귀 시 도안 페이지 1페이지로 초기화 | RC-03 표시 정합성 | **Major** (8/29 판정) | 8/23 (케이스 설계) | UI-51 실행 중 관찰 | 미수정 | **`d1c6010` (8/26, 시트 미반영)** | UI-51 | #36 |
| SPEC-PROJ-01 | 프로젝트 이름 길이 제한 미구현 (1~30자 신설) | 명세 공백 → 스펙 신설 | 심각도 축 제외 (8/29) | 8/23 (케이스 설계) | UI-17 설계 중 코드 3층 확인 | 미구현 | **`834876f` (8/26, 시트 미반영)** | UI-17 | #37 |
| DEF-20 | 저장되지 않은 rowCounter id 참조 시 500 누수 | 검증 공백 | **Major** (8/30 확정) | 8/29 (07 트랙) | API 원자성 주입 확인 중 발견 | 시트 미등록 | 미수정 | `test_05` 500 누수 케이스 | #38 |
| DEF-21 | 도안 연결 해제가 서버에 전파되지 않음 | RC-01 동기화 유실 | **Critical** (8/30 확정) | 8/5 (06 수동, 실기기) | TC-CUJ3-01 실행 중 발견 | 시트 미등록 | `e1b25ad` (8/10) | UI-46, TC-CUJ3-01 | #39 |
| DEF-22 | 필수 중첩 필드(rowCounter) 누락이 400 대신 500 누수 | 검증 공백 | **Major** (8/31 확정) | 8/31 (2차 07 API 트랙) | API-07 필수 필드 보강 테스트 작성 중 발견 | 시트 미등록 | 미수정 | `test_16_coverage_promotions.py::test_api_07_missing_row_counter_leaks_500_not_400` | 미등록 |
| DEF-23 | 앱 백그라운드 진입 시 작업 세션이 종료되지 않음 | RC-04 카운트 부정확 | **Critical** (9/1 확정) | 9/1 (정적 점검) | f93aacc 대조 중 앱 전체 scenePhase 부재 확인 | 시트 미등록 | 미수정 | 미정 (TIME-02 TC 신설 필요) | 미등록 |
| DEF-24 | 저장 충돌 안내 문구가 동작과 모순 ("다시 시도" 요구, 수정본은 폐기) | RC-03 표시 정합성 | Minor (9/1 확정) | 9/1 (정적 점검) | f93aacc 낙관적 잠금 대조 | 시트 미등록 | 미수정 | 미정 | 미등록 |
| DEF-25 | 창고 삭제 고지가 사용 기록 기준, 바늘과 도구는 고지 없음 | RC-03 표시 정합성 | Minor (9/1 확정) | 9/1 (정적 점검) | 기능정의서 정의 없음 14번 승인 중 코드 대조 | 시트 미등록 | 미수정 | 미정 | 미등록 |
| DEF-26 | 단위 옵션 값 체계가 화면별로 다름 (온보딩 US, 그 외 Imperial) | RC-04 데이터 계약 | Minor (9/1 확정, UI-03 기준) | 9/1 (정적 점검) | 기능정의서 도구, 설정, 홈 작성 중 대조 | 시트 미등록 | 미수정 | 미정 | 미등록 |
| DEF-27 | 프로필 편집의 표시 이름에 80자 클라 상한 없음 (회원가입엔 있음) | RC-04 데이터 계약 | Minor (9/1 확정) | 9/1 (정적 점검) | AppInputLimit 적용처 전수 확인 | 시트 미등록 | 미수정 | 미정 | 미등록 |
| DEF-28 | 스킬 테스트 저장 실패 문구가 개발자용 (Xcode 콘솔, 서버 Terminal) | RC-03 표시 정합성 | Minor (9/1 확정) | 9/1 (정적 점검) | 기능정의서 도구 작성 중 대조 | 시트 미등록 | 미수정 | 미정 | 미등록 |
| DEF-29 | 설정 단위 설명문이 미적용 사실과 어긋남 ("게이지와 길이 입력에서 사용") | RC-03 표시 정합성 | Minor (9/1 확정, UI-03 기준) | 9/1 (정적 점검) | UI-03 신설과 함께 확정 | 시트 미등록 | 미수정 | 미정 | 미등록 |
...
**회귀 감시.** `test_16_coverage_promotions.py::test_api_07_missing_row_counter_leaks_500_not_400`이 현재 동작(500)을 그대로 고정한다. 서버가 400으로 바뀌면 이 단언이 FAIL하고 그때가 가드가 채워졌다는 신호다.

---

### 2026-09-01 신규 7건 (DEF-23 ~ DEF-29) 심각도 근거

발견 시점은 전부 QA 신규 발견이고 방법은 정적 테스트(코드 대조)다. 실행 재현은 아직 없다. 대조 근거는 docs/spec/spec_drift_f93aacc.md 6절과 docs/spec/기능정의서_도구_설정_홈.md 불일치 목록이다.
