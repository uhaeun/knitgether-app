...
5. **빌드 진입점 불일치** — `package.json` 의 `start` 와 `ci.yml` 의 ios 잡이 `dist/main.js` 를 가리키는데 실제 빌드 산출물은 `dist/src/main.js` 다. `npm start` 가 로컬에서 동작하지 않는다. 제품 결함이 아니라 빌드 설정 문제라 대장에 올리지 않았으나 결정이 필요하다
