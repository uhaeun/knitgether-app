...
| DEF-22 | 필수 중첩 필드(rowCounter) 누락이 400 대신 500 누수 | 검증 공백 | **Major** (8/31 판정, 안 확정) | 8/31 (2차 07 API 트랙) | API-07 필수 필드 보강 테스트 작성 중 발견 | 시트 미등록 | 미수정 | `test_16_coverage_promotions.py::test_api_07_missing_row_counter_leaks_500_not_400` | 미등록 |
...
**심각도 판단 (초안, 확정 아님).** DEF-20과 같은 논리를 적용하면 Major다. 트랜잭션이 롤백돼 데이터는 온전하고(유실 아님), 계정 노출도 아니고, DEF-08처럼 다른 프로젝트까지 막는 앱 사용 불가 상태도 아니다. 다만 실제로 iOS 클라이언트가 `rowCounter`를 항상 채워 보내는 구조라면(`CLAUDE.md`에 기록된 "서버는 RowCounter가 옵셔널인데 클라 KnittingProject는 필수값" 불일치 참고) 사용자가 이 경로를 실제로 밟을 가능성 자체가 낮다는 점도 심각도 판단에 넣을지는 QA 몫이다.
