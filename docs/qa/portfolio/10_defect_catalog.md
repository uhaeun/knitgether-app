...
| DEF-20 | 저장되지 않은 rowCounter id 참조 시 500 누수 | 검증 공백 | **판단 필요** (8/29 신규) | 8/29 (07 트랙) | API 원자성 주입 확인 중 발견 | 시트 미등록 | 미수정 | `test_05` 500 누수 케이스 |
...
**남은 판단: 심각도.** 04 개정 기준으로 갈린다.

| 선택지 | 근거 |
| --- | --- |
| Major | 저장이 실패하므로 기능 실패다 |
| Minor | 트랜잭션이 롤백되어 데이터는 온전하고, 잘못된 것은 안내(500 대 4xx)뿐이다 |

데이터 유실도 계정 노출도 앱 사용 불가도 아니므로 Critical 상향 대상은 아니다.
...
4. **DEF-20 심각도** — Major와 Minor 중 어느 쪽인지(2-1절 뒤 상세 참조). 12 스코어카드에 반영되므로 확정 필요
5. **빌드 진입점 불일치** — `package.json` 의 `start` 와 `ci.yml` 의 ios 잡이 `dist/main.js` 를 가리키는데 실제 산출물은 `dist/src/main.js` 다. `tsconfig.build.json` 이 `scripts/` 를 제외하지 않아 rootDir이 서버 루트로 추론되기 때문이며, git에 `server/scripts/*.ts` 2개가 있으므로 CI에서도 같은 결과다(8/29 확인). `npm start` 와 ios 잡의 서버 기동이 동작하지 않는다. 고치는 방법이 둘이라 결정이 필요하다. 경로를 실제 산출물에 맞추거나, `scripts/` 를 빌드에서 빼 산출물을 `dist/main.js` 로 되돌리는 것이다. 제품 결함이 아니라 빌드 설정 문제라 대장에는 올리지 않았다
