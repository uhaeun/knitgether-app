...
> 3절의 25개 API 케이스와 11개 파일 37개 pytest 실행 항목은 8/30 시점 스냅샷이다. 9/19 기준 케이스 목록은 API-01~29(09), 7차 실행은 69 passed·0 failed(11의 11절)다. 케이스 수와 실행 항목 수는 서로 다른 단위다.
> 실행 코드는 [`qa/api-tests/`](../../../qa/api-tests/)에 있다.
> 06에서 이관받은 3건(SYNC-03, AUTH-01, RC-08)의 처리도 이 문서가 담당한다.

> 1, 2, 6, 7절의 분석 서술은 AI 초안에서 출발했다. 3~5절은 당시 실행 기록이며, 이후 실행 결과는 11에서 확인한다.
...
그래서 데이터 정합성을 판정하는 API 검증은 응답 코드에서 멈추지 않는다. 저장·삭제처럼 상태 변화를 확인해야 하는 케이스는 psql로 서버 DB를 대조하고, 파일을 다루는 케이스는 디스크까지 확인한다. 인증 거부처럼 응답 자체가 계약인 케이스는 응답 코드와 본문으로 판정한다.
...
데이터가 실제로 어떻게 남았는지는 2층에서 확인한다. 저장·삭제 결과를 판정할 때 DB 조회를 포함하는 이유다. 인증과 입력 거부처럼 응답 자체가 검증 대상인 케이스까지 DB 조회가 필수인 것은 아니다.
...
8/30 당시 09의 API 케이스 25건이 실행 코드 어디에 덮이는지 대조했다. 덮임 판정은 그 케이스의 검증 내용을 실제로 단언하는 테스트가 있는지 기준이고, 부수적으로 지나가기만 하는 것은 부분으로 본다. 이후 추가된 API-26~29의 대응은 09의 3절에 기록했다.
...
8/30 당시 미커버 5건은 시간이 없어서가 아니라 우선순위 때문에 남겼다.
...
당시 실행 결과는 36 passed, 1 skipped이었다. 스킵 1건은 Issue #15 특성화로 의도된 것이다.
...
8/30 시점의 미커버 5건은 3-2에 근거를 적었다. P2 이하이거나 치명도 축 밖이라 뺀 것이고, 이후 대응 상태는 09의 3절에서 갱신했다.
...
2026-09-15에 그 축적을 처음으로 수치화했다. 서버 저장소 194개 중 **182개(94%)가 DB에서 참조되지 않는다.** 역방향으로 DB가 참조하는데 디스크에 없는 것이 1건 있고, 이쪽은 고아화와 반대 방향이라 다운로드 실패로 이어진다.

숫자는 개발 환경 것이라 그대로 운영 수치로 읽으면 안 된다. 스위트를 반복 실행한 분량이 섞여 있다. 다만 **정리하는 주체가 아무도 없다**는 사실은 숫자와 무관하게 확인된다. 관측을 택한 대가가 무엇이었는지는 이 수치가 말해준다.
